...
const useStyles = makeStyles((theme) => ({
    container: {
        width: "550px", display: "flex", justifyContent: "center", alignItems: "center", flexDirection: "column"
    },
    devider: {
        width: "100%", height: "1px", margin: "10px 0px"
    },
    colorPalate: {
        width: "100px", height: "30px", marginTop: "10px", display: "flex", alignItems: "center", justifyContent: "center", borderRadius: "4px", cursor: "pointer"
    },
    buttonPosition: {
...
function BoardModel({ open, handleClose, handleSubmit, board }) {
    ...
    const [color, setColor] = useState({ color: board?.bgColor || colors.lightBlue, isPalateOpen: false })
    ...
    const _handleSubmit = useCallback(() => {
        handleSubmit({ titleError, title, color: { color: color.color, isPalateOpen: color.isPalateOpen } })
        handleClose()
    }, [color.color, color.isPalateOpen, handleClose, handleSubmit, title, titleError])
            ...
            <Box className={classes.container}>
                ...
                <Box style={{ width: "100%" }}>
                    <InputContainer
                        ...
                        autoFocus
                    />
                    <Box onClick={handleClickInColorPalate} className={classes.colorPalate} style={{ backgroundColor: color.color }}>
                        <TextElement font="bold" fontType="h8" textStyle={styles.text}>
                            {color.isPalateOpen ? "Done" : "Change Color"}
                        </TextElement>
                    </Box>

                    {color.isPalateOpen ? <SketchPicker
                        color={color.color}
                    ...
                    <Box className={classes.buttonPosition}>
                        <ButtonContainer customButtonStyle={{ ...styles.submitbutton, backgroundColor: color.color }} title={board ? "Edit Board" : "Create Board"} onClick={_handleSubmit} />
                    </Box>
                </Box>
            </Box>
